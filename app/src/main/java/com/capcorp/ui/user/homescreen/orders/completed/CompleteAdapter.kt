...
import com.capcorp.ui.user.homescreen.detail_offer_completed.detail_delivery.HomeCompltedUserActivity
...
import kotlinx.android.synthetic.main.item_order_completed_new.view.tvRequiredBy
...
//                var price = ""
//                    price = """${mContext.getString(R.string.currency_sign)} ${orderList[adapterPosition].accepted?.totalPrice}"""
//                val message = "${"Shopper+Traveller Delivery Reward"} :${price}}"
//
//                ViewTooltip.on(completedFragment.requireActivity(), itemView.ivInfo)
//                        .autoHide(true, 3000)
//                        .corner(30)
//                        .text(message)
...
//            }
            ...
            itemView.setOnClickListener {
                if (orderList.size > 0) {
                    val isCompleted = orderList.get(adapterPosition).insurance
                    if (orderList.get(adapterPosition).type.equals("Shop")) {
                        completedFragment.startActivityForResult(
                            Intent(mContext, HomeCompltedUserActivity::class.java)
                                .putExtra(
                                    COMPLETED_DETAIL,
                                    Gson().toJson(orderList[adapterPosition])
                                )
                                .putExtra("fromCompletedDetail", true)
                                .putExtra(IS_COMPLETED, false)
                                .putExtra(POSITION, adapterPosition), 400
                        )
                    } else if (orderList.get(adapterPosition).type.equals("Pickup")) {
                        completedFragment.startActivityForResult(
                            Intent(mContext, HomeCompltedUserActivity::class.java)
                                .putExtra(
                                    COMPLETED_DETAIL,
                    ...
                    } else if (orderList.get(adapterPosition).type.equals("Delivery")) {
                        completedFragment.startActivityForResult(
                            Intent(mContext, HomeCompltedUserActivity::class.java)
                                .putExtra(
                                    COMPLETED_DETAIL,
        ...
        fun bind(orderList: OrderListing) = with(itemView) {
            ...
            itemView.tvRequiredBy.text = context.getString(R.string.by) + " " +orderList.deliveredDate?.let { getOnlyDate(it, "EEE · MMM d") }
